...
export const getCropBadgesByUserType = ({ _id }, { badges, members }) => {
  /*
  GET MEMBER DATA OF USER IN CROP
  */
  const memberData = members.filter((member) => _id.toString() === member.user._id.toString())[0]

  /*
  VALIDATE USER TYPE
  */
  if(memberData.type !== UserTypes.MARKETER && memberData.type !== UserTypes.PROVIDER) return badges
