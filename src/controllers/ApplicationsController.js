...
class ApplicationsController {
  static async create(crop, stage) {
    ...
    const prodStage = await ProductionStage.findOne({ where: { production_id: production.id, label: stage } })
    const nextStage = await ProductionStage.findOne({ where: { order: prodStage.order + 1 } })
